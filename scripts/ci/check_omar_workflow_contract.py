...
def _reject_bridge_or_provider_inputs(text: str) -> None:
    ...
    for line in text.splitlines():
        ...
        if stripped.startswith("llm_provider:") and stripped != ALLOWED_LLM_PROVIDER_LINE:
            raise OmarWorkflowContractError(
                "Omar workflow must prefer OpenAI when configured, then Google, then managed SentinelLayer"
            )
...
def validate_omar_contract(workflow_text: str) -> None:
    ...
    required_direct_fragments = (
        ...
        "model_fallback: ${{ secrets.OPENAI_API_KEY != '' && 'gpt-4.1-mini' || ((secrets.GOOGLE_GEMINI_API_KEY != '' || secrets.GOOGLE_API_KEY != '') && 'gemini-2.5-flash' || 'gpt-4.1-mini') }}",
        ...
        "REQUESTED_FALLBACK_MODEL: ${{ secrets.OPENAI_API_KEY != '' && 'gpt-4.1-mini' || ((secrets.GOOGLE_GEMINI_API_KEY != '' || secrets.GOOGLE_API_KEY != '') && 'gemini-2.5-flash' || 'gpt-4.1-mini') }}",
