// Shared primitives for deterministic persona domain tools.
...
export function normalizeSeverity(value, fallback = "P2") {
  const normalized = String(value || "").trim().toUpperCase();
  if (SEVERITIES.includes(normalized)) {
    return normalized;
  }
  return SEVERITIES.includes(fallback) ? fallback : "P2";
}
...
export function createFinding(
  ...
  { defaultPersona = "", defaultKind = defaultPersona, severityFallback = "P2" } = {}
) {
  const findingPersona =
    String(persona || defaultPersona || "").trim() || "domain";
  return {
    persona: findingPersona,
    tool: String(tool || "").trim(),
    kind: String(kind || "").trim() || String(defaultKind || findingPersona).trim(),
    severity: normalizeSeverity(severity, severityFallback),
    file: toPosix(file || ""),
...
export async function* walkRepoFiles(
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entryMetadata === "stat") {
        yield { fullPath, relativePath, stat };
        continue;
      }
      if (entryMetadata === "sizeBytes") {
        yield { fullPath, relativePath, sizeBytes: stat.size };
        continue;
      }
...
export function evidenceAroundMatch(content, line) {
  return getLineContent(content, line);
}

export async function* iterateFiles(resolvedRoot, files, extensions) {
  if (Array.isArray(files) && files.length > 0) {
    for (const file of files) {
      ...
      yield { fullPath, relativePath, sizeBytes: stat?.size || 0 };
    }
    return;
  }

  yield* walkRepoFiles(
    { rootPath: resolvedRoot, extensions },
    { entryMetadata: "sizeBytes" }
  );
}
