 ...
 *
 * Location of the CLIs (first match wins): RESPAWN_CLI / RESPAWN_BUNDLE_CLI env; ./machine/dist/cli.js + ./bundle/dist/cli.js
...
function candidates(kind) {
  const rel = kind === "bundle" ? path.join("bundle", "dist", "cli.js") : path.join("machine", "dist", "cli.js");
  const envVar = kind === "bundle" ? process.env.RESPAWN_BUNDLE_CLI : process.env.RESPAWN_CLI;
  const list = [];
  ...
  const respawn = program
    ...
    .action(async () => {
      ...
      if (list[0] === "use") {
        ...
        console.log(JSON.stringify({ ok: true, repo: dir, machineCli: resolveRespawnCli("machine"), bundleCli: resolveRespawnCli("bundle") }));
        return;
      }
      const isBundle = list[0] === "bundle";
      const kind = isBundle ? "bundle" : "machine";
      ...
      if (!cli) {
        console.error(
          JSON.stringify({
            ...
            hint: "set RESPAWN_CLI / RESPAWN_BUNDLE_CLI, run inside a respawn checkout, `sl respawn use <repoDir>`, or run on a deployed orchestrator host (/opt/respawn/orch)",
      ...
      const forwarded = isBundle ? list.slice(1) : list;
