...
async function emitSentiEvent(
  ...
  const projectionOnly = normalizeString(event) === "daemon_alert";
  const envelope = createAgentEvent({
    ...
    payload: projectionOnly
      ? {
          ...payload,
          ephemeral: true,
          projection: "session_health",
        }
      : payload,
  });
  // Daemon health, presence, conflict hints, and lifecycle notices are
  // operational projections. They are returned to the caller/status surface,
  // but never become durable conversation rows.
  if (projectionOnly) {
    return envelope;
  }
